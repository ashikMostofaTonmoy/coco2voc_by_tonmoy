...
def coco2voc(anns_file, target_folder, n=None, compress=True):
    ...
    # print(type(coco_imgs))
    # print(f"coco_imgs: {coco_imgs}")
    ...
    for i, img in enumerate(coco_imgs):
        ...
        # print(f"anns: {anns}")
        # image_name = coco_instance.imgs['file_name']
        im = coco_instance.loadImgs(img)
        img_file_name = im[0]['file_name']
        # print(type(im[0]))
        # print(len(im))
        # print(f"im[0]['file_name']: {im[0]['file_name']}")
        # print(f"im[0]['file_name'] type: {type(im[0]['file_name'])}")
        print(f"imageId : {img} , file_name : {img_file_name}")
        if not anns:
            continue
        ...
        #     class_target_path + '/' + str(img) + '.png')
        # Image.fromarray(instance_seg).convert("L").save(
        #     instance_target_path + '/' + str(img) + '.png')
        ...
        Image.fromarray(class_seg).convert("L").save(
            class_target_path + '/' + str(img_file_name) + '.png')
        Image.fromarray(instance_seg).convert("L").save(
            instance_target_path + '/' + str(img_file_name) + '.png')

        if compress:
            np.savez_compressed(os.path.join(
                id_target_path, str(img_file_name)), id_seg)
        else:
            np.save(os.path.join(id_target_path, str(
                img_file_name)+'.npy'), id_seg)

        image_id_list.write(str(img_file_name)+'\n')
