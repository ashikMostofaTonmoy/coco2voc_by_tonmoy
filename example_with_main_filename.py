...
if __name__ == '__main__':
    ...
    annotations_file = "data/input/motorcycle (pixel) Sep 15 2022 17-46 Acme Project(instance)/COCO_motorcycle (pixel).json"
    labels_target_folder = 'data/output/motorcycle(instance)_test'
    # data_folder = '/home/dl/1TB-Volumn/MSCOCO2017/train2017'
    data_folder = 'data/input/motorcycle (pixel) Sep 15 2022 17-46 Acme Project(instance)/images'
...
""" 
   # Read ids of images whose annotations have been converted from specified file
    ...
    for id in id_list:
        ...
        id_seg = np.load(os.path.join(
            labels_target_folder, 'id_labels', id + '.npz'))
        segs.append(id_seg['arr_0'])

        # Example for loading class or instance segmentations
        instance_filename = os.path.join(
            labels_target_folder, 'instance_labels', id + '.png')
        class_filename = os.path.join(
            labels_target_folder, 'class_labels', id + '.png')
